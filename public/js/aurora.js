...
// the two screens read as one place. Its stop ALPHAS now mirror the green
// ramp exactly (0.08 / 0.42 / 0.9 / 0.5 / 0.26): the two colours are equals
// that share the strip, not a subject and its shadow.
const makeVioletRamp = (strength) =>
  makeRamp([
    [0, "rgba(150, 130, 255, 0)"],
    [0.1, `rgba(150, 130, 255, ${0.08 * strength})`],
    [0.24, `rgba(155, 120, 255, ${0.42 * strength})`],
    [CORE_AT, `rgba(198, 168, 255, ${0.9 * strength})`], // the bright core
    [0.55, `rgba(140, 110, 240, ${0.5 * strength})`],
    [0.78, `rgba(105, 80, 205, ${0.26 * strength})`], // the long veil
    [1, "rgba(90, 70, 190, 0)"],
  ]);
...
// sky always has one aurora at decent intensity; the other three come and
// go freely (so the count reads as 1–4).
const makeBand = (hero = false, violet = false) => ({
  hero,
  violet,
  ramp: violet
    ? makeVioletRamp(rand(0.8, 1.05))
...
// FOUR bands now, and green and violet are equals that share the sky — same
// ramp alphas, same thickness range, same brightness range. The hero (the one
// that never fades) stays green so the strip still opens on the signature
// colour, and the other three are dealt from a pool that always contains at
// least one of each: two violet on most loads, one on the rest. Which slot
// gets which is random, and every band keeps its own independent fade cycle,
// so the pairing you see drifts minute to minute.
const makeBandSet = () => {
  ...
  return [makeBand(true), ...pool.map((c) => makeBand(false, c === "violet"))];
};
