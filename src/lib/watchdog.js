    ...
    if (process.platform === "linux") {
      for (const pid of fs.readdirSync("/proc")) {
        ...
        try {
          ...
          if (m && /ffmpeg/i.test(m[1]) && st.split(") ")[1].split(" ")[1] === String(process.pid)) n++;
        } catch {}
      }
    }
  } catch {}
...
const tick = () => {
  ...
  if (lagMs > HARD_LAG_MS) lagStrikes++; else lagStrikes = 0;
  if (lagStrikes >= 3) { hardHeal(`event loop stuck (${lagMs} ms lag, 3 checks)`); lagStrikes = 0; return; }

  const soft = m.rss > SOFT_RSS || lagMs > SOFT_LAG_MS;
  if (soft && Date.now() - lastSoftAt > SOFT_COOLDOWN_MS) {
    softHeal(m.rss > SOFT_RSS ? `rss ${(m.rss / 1048576).toFixed(0)} MB` : `loop lag ${lagMs} ms`);
  }
